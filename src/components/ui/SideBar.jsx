...
const SideBarOverlay = React.forwardRef(({ className, ...props }, ref) => (
  <SideBarPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-50 bg-black dark:bg-neutral-800 opacity-70 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    {...props}
    ref={ref}
...
const SideBarVariants = cva(
  "fixed z-50 overflow-auto gap-4 bg-white px-4 py-3 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 dark:bg-neutral-950",
  {
    variants: {
      side: {
...
const SideBarContent = React.forwardRef(
  ({ side = "left", className, children, ...props }, ref) => (
    <SideBarPortal>
      ...
      >
        {children}
        {/* <SideBarPrimitive.Close className="absolute right-2 top-4 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-neutral-100 dark:data-[state=open]:bg-neutral-800">
...
const SideBarTitle = React.forwardRef(({ className, ...props }, ref) => (
  <SideBarPrimitive.Title
    ref={ref}
    className={cn(
      "text-lg font-semibold text-neutral-950 dark:text-neutral-50 text-start",
      className
    )}
    {...props}
  />
